    ...
    <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "#666" }}>{c.funil} · {TIPO_LABEL[c.tipoFunil] ?? c.tipoFunil}</p>
        </div>
      ...
      >
        ...
        <p style={{ fontSize: 12, color: "#666", maxWidth: 240, lineHeight: 1.6 }}>
          {c.status === "vermelho" && `${c.nomeKpi} acima da meta. Requer ação imediata para reduzir o custo de ${c.nomeKpi === "CPL" ? "lead" : "aquisição"}.`}
          {c.status === "amarelo" && `${c.nomeKpi} próximo do limite aceitável. Monitorar de perto nas próximas campanhas.`}
          {c.status === "verde"   && `${c.nomeKpi} dentro da meta. Campanha operando com eficiência.`}
        </p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 10 }}>
        <MetricBlock label="Resultados"     value={(c.resultados ?? 0).toLocaleString("pt-BR")} />
        <MetricBlock label="Investimento"   value={`R$ ${c.investimento.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`} />
        <MetricBlock label={c.nomeKpi}      value={`R$ ${(c.kpi ?? 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`} accent={statusColor} />
      </div>
